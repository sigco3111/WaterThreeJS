...
const fPre = gui.addFolder(t('folderCinematic')).close();
const presetProxy = { preset: t('presetTropicalNoon') };
fPre.add(presetProxy, 'preset', Object.keys(PRESETS)).name(t('presetLabel')).onChange(applyPreset);
fPre.add({ cinema: false }, 'cinema').name(t('cinematicCamera'))
  .onChange((v) => (controls.autoRotate = v));

const fSun = gui.addFolder(t('folderTimeOfDay')).close();
fSun.add(sunParams, 'elevation', -3, 89, 0.5).name(t('sunElevation')).onChange(applySun);
fSun.add(sunParams, 'azimuth', 0, 360, 1).name(t('sunAzimuth')).onChange(applySun);

const fWaves = gui.addFolder(t('folderWaves')).close();
fWaves.add(ocean.uniforms.uAmplitude, 'value', 0.1, 3.5, 0.05).name(t('amplitude'));
fWaves.add(ocean.uniforms.uChoppy, 'value', 0.0, 1.4, 0.02).name(t('choppiness'));
fWaves.add(ocean.uniforms.uWaveCount, 'value', 4, 40, 1).name(t('waveCount'));
fWaves.add(ocean.uniforms.uSpeed, 'value', 0.0, 3.0, 0.05).name(t('speed'));
fWaves.add(ocean.uniforms.uDirSpread, 'value', 0.0, 1.6, 0.02).name(t('directionSpread'));
fWaves
...
const fSurf = gui.addFolder(t('folderSurface')).close();
fSurf.add(ocean.uniforms.uDetailStrength, 'value', 0.0, 1.2, 0.02).name(t('rippleDetail'));
fSurf.add(ocean.uniforms.uDetailScale, 'value', 0.05, 1.2, 0.01).name(t('rippleScale'));
fSurf.add(ocean.uniforms.uRefractStrength, 'value', 0.0, 0.12, 0.005).name(t('refraction'));
fSurf.add(ocean.uniforms.uSSRStrength, 'value', 0.0, 1.0, 0.02).name(t('reflectionsSSR'));
fSurf.add(ocean.uniforms.uSunGlitter, 'value', 0.0, 1.0, 0.02).name(t('sunGlitter'));
fSurf.add(ocean.uniforms.uRoughness, 'value', 0.02, 0.5, 0.01).name(t('microRoughness'));

const fColor = gui.addFolder(t('folderColor')).close();
fColor.add(ocean.uniforms.uClarity, 'value', 0.3, 3.0, 0.05).name(t('clarity'));
fColor.add(ocean.uniforms.uDepthFalloff, 'value', 0.03, 0.5, 0.01).name(t('depthFalloff'));
fColor.add(ocean.uniforms.uSSSStrength, 'value', 0.0, 1.5, 0.02).name(t('translucency'));
addColorCtrl(fColor, ocean.uniforms.uShallowColor, t('shallow'));
addColorCtrl(fColor, ocean.uniforms.uDeepColor, t('deep'));
addColorCtrl(fColor, ocean.uniforms.uFoamColor, t('foam'));

const fFoam = gui.addFolder(t('folderFoam')).close();
fFoam.add(ocean.uniforms.uFoamCoverage, 'value', 0.0, 2.0, 0.05).name(t('coverage'));
fFoam.add(ocean.uniforms.uFoamEdge, 'value', 0.02, 0.45, 0.01).name(t('softnessLayers'));
fFoam.add(ocean.uniforms.uFoamOpacity, 'value', 0.3, 1.0, 0.02).name(t('opacity'));
fFoam.add(ocean.uniforms.uCrestFoamStart, 'value', 0.3, 3.0, 0.05).name(t('whitecapOnset'));
fFoam.add(ocean.uniforms.uFoamThreshold, 'value', 0.0, 1.0, 0.02).name(t('breakingFoam'));
fFoam.add(ocean.uniforms.uShoreFoamWidth, 'value', 0.0, 8.0, 0.1).name(t('shoreFoamWidth'));
fFoam.add(ocean.uniforms.uContactFoam, 'value', 0.0, 2.0, 0.05).name(t('objectFoamWakes'));

const fObj = gui.addFolder(t('folderObjects')).close();
fObj.add({ s: () => dropAtTarget('sphere') }, 's').name(t('dropSphere'));
fObj.add({ c: () => dropAtTarget('cube') }, 'c').name(t('dropCube'));
fObj.add({ x: () => bodies.clear() }, 'x').name(t('clearObjects'));
fObj.add(bodies, 'gravity', 0, 45, 0.5).name(t('gravity'));

const fClouds = gui.addFolder(t('folderClouds')).close();
const cu = clouds.uniforms;
fClouds.add({ on: true }, 'on').name(t('enabled')).onChange(setCloudsEnabled);
fClouds.add(cu.uSteps, 'value', 16, 80, 2).name(t('qualitySteps'));
fClouds.add(cu.uCoverage, 'value', 0.1, 0.95, 0.01).name(t('coverage'));
fClouds.add(cu.uDensity, 'value', 0.2, 3.0, 0.05).name(t('density'));
fClouds.add(cu.uNoiseScale, 'value', 0.002, 0.02, 0.0005).name(t('cloudSizeInv'));
fClouds.add(cu.uHeightFalloff, 'value', 0.0, 1.0, 0.02).name(t('roundness'));
fClouds.add(cu.uDetail, 'value', 0.0, 1.0, 0.02).name(t('wispiness'));
fClouds.add(cu.uBase, 'value', 120, 900, 10).name(t('altitude'));
fClouds.add(cu.uHeight, 'value', 100, 700, 10).name(t('thickness'));
fClouds.add(cu.uWindSpeed, 'value', 0.0, 0.15, 0.005).name(t('windSpeed'));
fClouds.add(cu.uSunStrength, 'value', 0.5, 6.0, 0.1).name(t('sunStrength'));
fClouds.add(cu.uAmbient, 'value', 0.0, 1.2, 0.02).name(t('ambient'));
fClouds.add(cloudShadowP, 'strength', 0.0, 1.0, 0.02).name(t('seaShadows'));

const fUnder = gui.addFolder(t('folderUnderwater')).close();
fUnder.add(post.underwaterMat.uniforms.uShaftDensity, 'value', 0.0, 0.2, 0.005).name(t('godRayDensity'));
fUnder.add(post.underwaterMat.uniforms.uFogStrength, 'value', 0.0, 2.0, 0.05).name(t('fogStrength'));

const fPost = gui.addFolder(t('folderPost')).close();
fPost.add(post.compositeMat.uniforms.uExposure, 'value', 0.3, 2.0, 0.02).name(t('exposure'));
fPost.add(post.compositeMat.uniforms.uBloom, 'value', 0.0, 2.0, 0.02).name(t('bloom'));
fPost.add(post, 'bloomStreak', 0.0, 1.0, 0.02).name(t('anamorphicStreak'));
fPost.add(post.compositeMat.uniforms.uSaturation, 'value', 0.5, 1.6, 0.02).name(t('saturation'));
fPost.add(post.compositeMat.uniforms.uContrast, 'value', 0.8, 1.3, 0.01).name(t('contrast'));
fPost.add(post.compositeMat.uniforms.uGrain, 'value', 0.0, 0.2, 0.005).name(t('filmGrain'));
fPost.add(post.compositeMat.uniforms.uCA, 'value', 0.0, 2.0, 0.05).name(t('lensFringe'));
fPost.add(post.compositeMat.uniforms.uVignetteAir, 'value', 0.0, 0.6, 0.02).name(t('vignette'));

gui.add({ dive: () => diveTo(-12) }, 'dive').name(t('diveUnder'));
gui.add({ surface: () => diveTo(14) }, 'surface').name(t('backToSurface'));
...
function animate() {
  ...
  depthStateEl.textContent = underwater ? t('stateBelow') : t('stateAbove');
  depthStateEl.style.color = underwater ? '#7fe0d0' : '#9be7ff';
  depthValEl.textContent = (underwater ? depthBelow : camera.position.y).toFixed(1) + t('depthUnit');
